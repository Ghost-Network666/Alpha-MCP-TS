...
export function getAgentRecipes(): Record<string, unknown> {
  return {
    ...
    completeIntentCoverage: 'NO COMMITS UNTIL GATE SATISFIED: route_agent_intent now supplies complete intent-language routing for 100% of native MCP tools (43+ intents covering every public/secure/intel/strategy/advanced/meta/weather/portfolio/trading/cancels/on-chain/gasless/prepare/leaderboard/comment/taxonomy/sports/series/rewards-earnings tool). Every native tool name appears in at least one plan returned by route_agent_intent. All plans start with fetch_sdk_readme + get_agent_recipes + get_strategies, include explicit steps + rich agentDirective ("re-call route_agent_intent, NEVER guess the next tool name/sequence/params from descriptions alone"), nextTools, and the COMPLETE COVERAGE note. See INTENT_REGISTRY and buildIntentRoute in src/mcp/intent-routing.ts. The meta_introspection_full, *_full, *_deep, *_complete etc. intents exist precisely so agents can reach any tool via deterministic routing without guessing. This is the "all mcp tools have intent language routing, allowing all native mcp tools completely route the agent to never guessing" requirement. The continuous improvement ritual includes an explicit completeness audit + local node.exe verification before any commit.',
    placeLimitOrder: {
