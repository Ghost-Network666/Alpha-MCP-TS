...
- In this workspace, verification is always pure native (`node -e '...'` spawning dist/mcp.js over pipes with full init + calls, zero committed test .js files; all in /tmp or ephemeral only). Never commit machine-local paths or usernames.
...
2. **Run the MCP locally "in here /mcp as its loaded in grok now"**: 
   ...
   - Also run a **fresh local execution** of the current dist (`node -e '...'` or full stdio pipe handshake + calls over the built dist/mcp.js, all ephemeral — never commit test scripts or local paths).
...
**Synced to latest ts-sdk/main (Jun 2026 commits)**: MCP explicitly aligned for: feat default secure client to deposit wallet (auto-deploy for non-EOA, config.wallet optional), make trading approvals idempotent (setupTradingApprovals repeatable/safe), refactor/clarify deposit wallet setup flow + EOA approval integration tests, docs mark action helpers low-level (PR#79), tags/series contract alignment + drop un-honored params (PR#78). Client creation (createPublicClient/createSecureClient + .extend(allActions) in src/config/client.ts), llms-guide.ts (buildMcpLlmsGuide + MCP_CATEGORIES), prompts (mcp_llms_full_guide + structure), AGENTS, and runtime usage already reflect these (no behavior change to tools; this doc/guidance refresh ensures agents get non-stale info). Published @beta remains 0.1.0-beta.2 (canaries track main). Re-verify native stdio (categories + prompts/get + get_mcp_usage) + full host restart after any SDK bump per critical reload note.
