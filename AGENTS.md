...
**Note on .extend(allActions)**: MCP now does this explicitly on both public and secure clients (see src/config/client.ts) so the full ~50+ decorated methods (listMarkets, placeLimitOrder, listSports, fetchTag, listBuilder*, getOrderBook, listActivity, fetchPortfolioValue, setupGaslessWallet, isGaslessReady, setupTradingApprovals, etc.) are available internally. Some balance flows still use standalone from @polymarket/client/actions for post-gasless reliability (the client instance shape can change).
